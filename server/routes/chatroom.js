...
router.put("/fix-chatroom", (req, res) => {
  const { domainID } = req.body;
  let count = 0;
  Chatroom.updateMany({ domain: null }, { domain: domainID })
    .then((updateInfo) => {
      const { nModified } = updateInfo;
      res.status(200).send({
        success: true,
        message: `${nModified} rooms updated!`,
      });
    })
    .catch((error) => {
      res.status(500).send({
...
router.get("/random/:userID", (req, res) => {
  ...
  Chatroom.countDocuments({
    $or: [
      {
        $and: [{ done: 0 }, { $or: [{ user1: userID }, { user2: userID }] }],
      },
      {
        $and: [
          { done: 0 },
          {
            $or: [
              {
                $and: [{ user1: null }, { client: userID }],
              },
              {
                $and: [{ user2: null }, { servant: userID }],
              },
            ],
          },
        ],
    ...
    if (err)
      res.status(500).send({
        success: false,
        message: "Can't estimate room document count",
        err,
      });
    // Get a random entry
    ...
    Chatroom.findOne({
      ...
      .exec((err, roomFound) => {
        if (err)
          res.status(500).send({
            success: false,
            message: "Can't proceed to find any room",
            err,
          });
        return res.status(200).send({
          success: true,
...
router.get("/getRoomByUserID/:userID", (req, res) => {
  ...
  Chatroom.find({
    $or: [{ client: userID }, { servant: userID }],
  })
    .then((batchRoomFound) => {
      let result = [];
      let count = 0;
        ...
        if (err.code === 11000) {
          Chatroom.create({ name: `${name}_1`, task, content_type })
            ...
            .catch((err) => {
              res.status(500).send({
                success: false,
                error: "Can't create room! Please try again after 2 seconds!!",
              });
            });
        }
  ...
  Chatroom.findById(roomID)
    ...
    .then((roomUpdated) => {
      if (!roomUpdated) {
        res.status(500).send({
          success: false,
          message: "Can't update room information after upload audio!!!",
        });
    ...
    .catch((err) => {
      res.status(500).send({
        success: false,
        message: "Unable to update audio information to room",
      });
      console.log(err);
      throw err;
    });
...
router.delete("/delete/:roomID", (req, res) => {
  ...
  Chatroom.findByIdAndDelete(roomID).then((roomFound) => {
    if (!roomFound)
      res.status(404).send({ success: false, message: "Room not found" });
    else
      res
        .status(201)
        .send({ success: true, message: "Room deleted!", roomFound });
  });
});

router.put("/finish/:roomID", (req, res) => {
  const { roomID } = req.params;
  Chatroom.findById(roomID).then((roomFound) => {
    if (!roomFound)
      res.status(404).send({ success: false, message: "Room not found" });
    else {
      roomFound.done = true;
...
router.put("/:roomID/:userRole", (req, res) => {
  ...
  Chatroom.findById(roomID).then(async (roomFound) => {
    if (!roomFound)
      res.status(404).send({ success: false, message: "Room not found" });
    else {
      // check turn
      if (
        !(
          (userRole === "client" && roomFound.turn === 1) ||
          (userRole === "servant" && roomFound.turn === 2)
        )
      ) {
        res
          .status(409)
          .send({ success: 0, message: "You have to wait for your turn!" });
        return;
      }
      ...
      if (roomFound.audioList.length === 0) {
        res
          .status(406)
          .send({ success: -1, message: "There's nothing to delete!" });
        return;
      }
      ...
      if (roomFound.turn === 1) {
        ...
        res.status(200).send({
          success: 0,
          message:
            "How did you even do this...? Removing audio... Update room turn... ",
        });
        return;
      }
      ...
      return roomFound.save((err, roomUpdated) => {
        ...
        return res
          .status(200)
          .send({ success: 1, message: "Remove audio successfully!" });
      });
    }
  });
});
